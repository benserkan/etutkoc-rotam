...
from app.models.weekly_skeleton import SkeletonGhostAction, WeeklySkeletonSlot
...
def _accept(
    ...
    section_id: int, count: int, chip_rank: int | None, chip_kind: str | None,
    chip_count: int | None, warnings: list[str],
):
    ...
    payload = TaskCreateBody(
        date=d.isoformat(), type="test", title="Görev", period=slot.period,
        items=[TaskItemBody(
    ...
    db.add(SkeletonGhostAction(
        ...
        section_id=sec.id, topic_id=sec.topic_id, task_id=task.id,
    ))
    return task
...
def accept_ghost(
    ...
    try:
        task = _accept(
            db, user=user, student=student, slot=slot, d=d, section_id=body.section_id,
            count=body.count, chip_rank=body.chip_rank, chip_kind=body.chip_kind,
            chip_count=body.chip_count, warnings=warnings,
        )
...
def accept_routine(
    ...
    """O günün RUTİN hayaletlerini ilk çiple topluca onayla."""
    student = _get_owned_student(db, student_id, user.id)
    assert_active_coaching(db, user)
    d = _parse_iso_date(body.date)
    data = sk.build_ghosts(db, student=student, coach_id=user.id, start=d, end=d)
    routine = [g for day in data["days"] for g in day["ghosts"] if g["is_routine"] and g["chips"]]
    warnings: list[str] = []
    tasks = []
    try:
        for g in routine:
            ...
            tasks.append(_accept(
                db, user=user, student=student, slot=slot, d=d, section_id=c["section_id"],
                count=c["count"], chip_rank=1, chip_kind=c["kind"],
                chip_count=len(g["chips"]), warnings=warnings,
            ))
    except ReservationError as e:
        db.rollback()
        raise _reservation_to_http(e)
