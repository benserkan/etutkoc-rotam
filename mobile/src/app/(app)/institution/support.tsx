...
import { createSupportRequest, getMyRequests, getSupportInbox, supportKeys } from "@/lib/support";

export default function InstitutionSupportScreen() {
  const qc = useQueryClient();
  const [view, setView] = React.useState<"mine" | "inbox">("inbox");
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50">
      <View className="px-5 pb-1 pt-3">
        <Text className="text-2xl font-extrabold text-slate-900">Talepler</Text>
      </View>
      {active.isLoading ? (
