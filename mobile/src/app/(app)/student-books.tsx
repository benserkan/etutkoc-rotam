...
import { getStudentBooksProgress, studentDevKeys } from "@/lib/student";

export default function StudentBooksRoute() {
  const q = useQuery({ queryKey: studentDevKeys.books, queryFn: getStudentBooksProgress });
  ...
  return (
    <SafeAreaView edges={["top"]} className="flex-1 bg-slate-50">
      <View className="flex-row items-center gap-1 px-2 py-2">
        ...
        <Text className="text-base font-semibold text-slate-800">Kitaplarım</Text>
      </View>
      ...
      {q.isLoading ? (
        ...
        <BooksView data={q.data} refreshing={q.isRefetching} onRefresh={() => q.refetch()} />
      )}
    </SafeAreaView>
  );
}
