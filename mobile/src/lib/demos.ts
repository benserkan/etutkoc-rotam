/**
 * Demo Kütüphanesi — mobil (web/lib/demos.ts ile AYNI veri).
 *
 * Her demo: rol + contextKey (uygulamada NEREYİ anlattığı). Panel-içi
 * "▶ Nasıl kullanılır?" rozeti (DemoHint) contextKey ile eşleşince çıkar;
 * dokununca demo, uygulama-içi tarayıcıda (expo-web-browser) açılır:
 *   {API_BASE}/demos?play={slug}  (sahne + sesli anlatım, web oynatıcı).
 *
 * Yeni demo eklenince web + mobil registry birlikte güncellenir.
 */
...
/** Bağlam + rol için yayınlanmış demo (yoksa null → rozet görünmez). */
...
/** Bir rolün yayınlanmış demoları (sırayla). */
...
/** Uygulama-içi tarayıcıda açılacak tam URL. */
export function demoPlayUrl(slug: string): string {
  return `${API_BASE}/demos?play=${encodeURIComponent(slug)}`;
}
