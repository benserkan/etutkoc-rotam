...
import type { TeacherRequestListItem, TeacherRequestListResponse } from "@/lib/teacher";
...
function RequestCard({
  ...
  return (
    <View className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      {proposed.length > 0 ? (
        <Text className="mt-1 text-[13px] text-slate-700">Öğrencinin önerisi: {proposed.join(" · ")}</Text>
      ) : null}

      {req.message ? <Text className="mt-2 text-sm text-slate-700">{req.message}</Text> : null}

      {req.teacher_response ? (
        <View className="mt-2.5 rounded-xl bg-cyan-50 p-3">
          <Text className="text-[11px] font-semibold uppercase tracking-wide text-cyan-700">Yanıtın</Text>
          ...
          {isQuestion ? (
            ...
            <>
              <Pressable
                ...
                disabled={busy}
                className="flex-1 flex-row items-center justify-center gap-1.5 rounded-xl bg-emerald-600 py-2.5 active:bg-emerald-700"
              >
                <Ionicons name="checkmark" size={18} color="#fff" />
                <Text className="text-sm font-semibold text-white">Onayla</Text>
              </Pressable>
              <Pressable
                onPress={() => onReject(req)}
                disabled={busy}
                className="flex-1 flex-row items-center justify-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 py-2.5 active:bg-rose-100"
              >
                <Ionicons name="close" size={18} color="#e11d48" />
                <Text className="text-sm font-semibold text-rose-700">Reddet</Text>
              </Pressable>
            </>
          )}
        </View>
      ...
      <Text className="mt-2 text-[11px] text-slate-400">{shortDate(req.created_at.slice(0, 10))}</Text>
    </View>
  );
}
...
export function TeacherRequestsView({
  ...
  data: TeacherRequestListResponse;
  busy?: boolean;
  onApprove: (r: TeacherRequestListItem) => void;
  onReject: (r: TeacherRequestListItem) => void;
  onRespond: (r: TeacherRequestListItem) => void;
    ...
    >
      {data.items.length === 0 ? (
        <View className="mt-10 items-center gap-3 px-6">
          <Ionicons name="checkmark-done-outline" size={44} color="#94a3b8" />
          <Text className="text-center text-base font-semibold text-slate-700">Bekleyen talep yok</Text>
          <Text className="text-center text-sm text-slate-500">
            Öğrencilerin program değişikliği veya soru ilettiğinde burada görünür.
          </Text>
        </View>
      ) : (
        data.items.map((r) => (
